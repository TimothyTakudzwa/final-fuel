...
def delivery_schedules(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('export_to_csv')=='csv':
            ...
            with open(f'{download_file}.pdf', 'rb') as pdf:
                response = HttpResponse(pdf.read(), content_type="application/vnd.pdf")
                response['Content-Disposition'] = f'attachment;filename={export_name} - Delivery Schedules - {today}.pdf'
                return response        
...
'''
function to retrieve notifications for the notification panel and bell
'''
...
def view_pop(request, id):
    ...
    return render(request, 'buyer/proof_of_payment.html', {'payment_history' : payment_history})